...
4. **Transcript delivery**: Either immediate dispatch or confirm-on-device (`action_send`/`action_undo`). The `desktop_mic` source always forces immediate dispatch (see `resolveSegmentOptions` in server.mjs) — it has no on-device confirm UI. For `xiaomi_remote`, `confirm_on_device` becomes a `remote_preview`: the transcript is shown in the desktop floating overlay first, the remote's OK click injects and sends it in one step (injection uses the configured `TEXT_INJECTION_MODE`), and Back discards it — the focused input is never touched before confirmation. A `prompt` button action arms a template so the next voice transcript is wrapped into its `{text}` placeholder before sending.
