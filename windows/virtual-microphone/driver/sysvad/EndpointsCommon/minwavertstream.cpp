...
// The product exposes one render endpoint (written by the desktop helper) and
// one capture endpoint (opened by WeChat). Both endpoints use 16 kHz mono
// PCM16, so a bounded nonpaged byte ring is sufficient to model the cable.
// Position/timing remain owned by the ordinary WaveRT stream implementation.
static const ULONG g_VibeCableCapacity = 16 * 1024 * 2;
static UCHAR g_VibeCableBuffer[g_VibeCableCapacity] = {};
static KSPIN_LOCK g_VibeCableLock = 0;
static ULONG g_VibeCableRead = 0;
static ULONG g_VibeCableWrite = 0;
static ULONG g_VibeCableCount = 0;

static
BOOLEAN
VibeCableFormatSupported
(
    _In_ PWAVEFORMATEXTENSIBLE Format
)
{
    return Format != NULL &&
           Format->Format.nChannels == 1 &&
           Format->Format.nSamplesPerSec == 16000 &&
...
{
    KIRQL oldIrql;
    KeAcquireSpinLock(&g_VibeCableLock, &oldIrql);
    g_VibeCableRead = 0;
    g_VibeCableWrite = 0;
    g_VibeCableCount = 0;
    KeReleaseSpinLock(&g_VibeCableLock, oldIrql);
}
...
)
{
    ...
    if (ByteCount > g_VibeCableCapacity)
    {
        Source += ByteCount - g_VibeCableCapacity;
        ByteCount = g_VibeCableCapacity;
    }
    ...
    ULONG overflow = 0;
    if (g_VibeCableCount + ByteCount > g_VibeCableCapacity)
    {
        overflow = g_VibeCableCount + ByteCount - g_VibeCableCapacity;
        g_VibeCableRead = (g_VibeCableRead + overflow) % g_VibeCableCapacity;
        g_VibeCableCount -= overflow;
    }

    ULONG remaining = ByteCount;
    while (remaining > 0)
    {
        ULONG run = min(remaining, g_VibeCableCapacity - g_VibeCableWrite);
        RtlCopyMemory(g_VibeCableBuffer + g_VibeCableWrite, Source, run);
        Source += run;
        remaining -= run;
        g_VibeCableWrite = (g_VibeCableWrite + run) % g_VibeCableCapacity;
    }
    g_VibeCableCount += ByteCount;
...
VibeCablePop
(
    _Out_writes_bytes_(ByteCount) UCHAR* Destination,
    _In_ ULONG ByteCount
)
{
    KIRQL oldIrql;
    ...
    ULONG copied = min(ByteCount, g_VibeCableCount);
    ULONG remaining = copied;
    while (remaining > 0)
    {
        ULONG run = min(remaining, g_VibeCableCapacity - g_VibeCableRead);
        RtlCopyMemory(Destination, g_VibeCableBuffer + g_VibeCableRead, run);
        Destination += run;
        remaining -= run;
        g_VibeCableRead = (g_VibeCableRead + run) % g_VibeCableCapacity;
    }
    g_VibeCableCount -= copied;
...
)
{
    ...
    else
    {
        ...
        // Consume frames from the render DMA buffer into the virtual cable.
        // Optional sample file tracing remains handled inside ReadBytes.
...
--*/
{
    ...
    // Capture consumes the render timeline. Underflow is silence, which keeps
    // WaveRT timing stable before and after a push-to-talk session.
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        if (VibeCableFormatSupported(m_pWfExt))
        {
            ULONG copied = VibeCablePop(m_pDmaBuffer + bufferOffset, runWrite);
            if (copied < runWrite)
            {
...
--*/
{
    ...
    // Normally this loops no more than once for a single wrap.
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        if (VibeCableFormatSupported(m_pWfExt))
        {
            VibeCablePush(m_pDmaBuffer + bufferOffset, runWrite);
        }
